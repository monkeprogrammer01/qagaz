...
const ApplicationCard = ({ application, onAssignAdmin, onMarkComplete }) => {
  const adminLabel =
    application?.admin?.email || application?.admin?.name || null;
    ...
    <div className="booking-item">
      ...
      <div className="booking-info">
        ...
        <div className="info-line">
          ...
          <span className="info-value">{formatTime(application.time)}</span>
        </div>

        {adminLabel && (
          <div className="info-line">
            <span className="info-key">Admin</span>
            <span className="info-value">{adminLabel}</span>
          </div>
        )}
      </div>
      ...
      {(application.status === "new" || application.status === "assigned") && (
        <div className="booking-footer">
          {application.status === "new" && (
            <button className="btn btn-primary" onClick={onAssignAdmin}>
              Assign Admin
            </button>
          )}
